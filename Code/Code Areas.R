...
# Check normality of area by age group and region
normality_results <- data %>%
  group_by(Age, Region) %>%
  summarise(normal = check_normality(Area))
...
# Calculate the central tendency (mean or median) of the area for each age group and region
central_measures <- data %>%
  group_by(Age, Region) %>%
  summarise(Area = if (use_mean) mean(Area) else median(Area))

# Transform data for radar chart plotting
data_radar <- central_measures %>%
  spread(key = Region, value = Area) %>%
  ungroup()
...
jpeg("radar_areas_sept.jpg", width = 2050, height = 1200, res = 300)
...
png("radar_area_color2.png", width = 1200, height = 700, res = 200)
...
title(main = "Central tendency values of areas of the epidermis across different regions and ages", cex.main = 0.8)
