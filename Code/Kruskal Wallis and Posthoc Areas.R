...
kruskal_results <- data %>%
  group_by(Region) %>%
  kruskal_test(Area ~ Age)

# Save Kruskal-Wallis test results to a CSV file
write.csv(kruskal_results, "kruskal_Results_Area.csv", row.names = FALSE)
...
posthoc_results <- data %>%
  ...
  group_by(Region) %>%
  dunn_test(Area ~ Age, p.adjust.method = "bonferroni")

# Save post-hoc test results to a CSV file
write.csv(posthoc_results, "posthoc_cells Area.csv", row.names = FALSE)
